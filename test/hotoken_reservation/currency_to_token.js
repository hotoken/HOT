...
contract('HotokenReservation', function(accounts) {
  describe.only('currency->token', function() {
    ...
    it('1ETH/$500/65% buy:1ETH', async function() {
      await h.setConversionRate('ETH', 50000) // 1ETH = $500.00
      await h.setDiscountRate(3)

      let tokens = await h.applyDiscount(await h.usdToTokens(await h.toUsd('ETH', 1*10**18)))
    ...
    it('1ETH/$500/65% buy:0.34ETH', async function() {
      ...
      let tokens = await h.applyDiscount(await h.usdToTokens(await h.toUsd('ETH', 34*10**16)))
    ...
    it('1ETH/$500/65% buy:22ETH', async function() {
      ...
      let tokens = await h.applyDiscount(await h.usdToTokens(await h.toUsd('ETH', 22*10**18)))
      expect(tokens.toNumber()).to.be.equal(181500*10**18)
    })

    it('1ETH/$380.25/65% buy:1ETH', async function() {
      await h.setConversionRate('ETH', 38025)
      await h.setDiscountRate(3)
